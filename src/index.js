...
export default class ImageViewer extends Component {
      ...
      onPanResponderRelease: (evt, gestureState) => {
        if (evt.nativeEvent.changedTouches.length <= 1) {
          if (this.isClick) {
            if (
              this.lastClickTime &&
              new Date().getTime() - this.lastClickTime < 300
            ) {
   ...
   */
  getImageList() {
    let { imageUrls, headers = {} } = this.props;
            ...
            <View style={viewer.failedImg} key={index}>
              {this.props.failedUrl ? (
                <Image
                  ...
                  source={{ uri: this.props.failedUrl, ...headers }}
                />
              ) : (
                <Image
                  style={viewer.failedImg}
  ...
  render() {
    ...
    return (
      <Modal
        ...
        transparent={true}
        animationType={"none"}
        onRequestClose={this.modalDismissed.bind(this)}
      >
        <Animated.View
          style={[
            viewer.titleBar,
    ...
    this.setState(
      ...
      () => {
        ...
        this.standardPositionX =
          len % 2 === 0 ? this.positionX + screenWidth / 2 : this.positionX;
